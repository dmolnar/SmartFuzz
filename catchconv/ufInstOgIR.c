...
ufOgNode * tmpNametoNode(HWord tmpName, HWord transNum, HWord execNum)
{
  ...
  if (hashret == NULL )
  {
    ...
    hashretH = VG_(malloc)("ufInstOgIR", sizeof(ufOgHashNode)); 
    hashretLive = VG_(malloc)("ufInstOgIR", sizeof(ufOgHashNode)); 
...
void ufInstOgIRStore (IRSB * bb,  IRStmt* s , ULong bb_counter)
{
  ...
  case Iex_RdTmp:
    ...
    {
      dataTmpName = (HWord)dataExpr->Iex.RdTmp.tmp; 
  ...
  case Iex_Const:
    ...
    if (dataExpr->tag == Iex_RdTmp)
    {
      dataTmpName = dataExpr->Iex.RdTmp.tmp; 
...
void ufInstOgIRTemp (IRSB * bb,  IRStmt* s , ULong bb_counter)
{
  ...
  lhsTmpName = (HWord)s->Ist.WrTmp.tmp; 
  ...
  case (Iex_RdTmp):
    rhsTmpName = (HWord)(s->Ist.WrTmp.data->Iex.RdTmp.tmp); 
  ...
  case (Iex_Unop):
    ...
    if(rhs->Iex.Unop.arg->tag == Iex_RdTmp)
    { 
      rhsTmpName = (HWord)rhs->Iex.Unop.arg->Iex.RdTmp.tmp;
  ...
  case (Iex_Binop):
    ...
    if (argX->tag == Iex_RdTmp)
    {
      argXTmpName = (HWord)argX->Iex.RdTmp.tmp;
    }

    if (argY->tag == Iex_RdTmp)
    {
      argYTmpName = (HWord)argY->Iex.RdTmp.tmp; 
    }
