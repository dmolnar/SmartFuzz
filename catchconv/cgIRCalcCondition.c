...
void 
cgAddTmpCCall(IRSB * bb, IRStmt * s)
{
   HWord lhsTmp;
   IRExpr * call; 
   IRDirty * d; 
   ...
   if (VG_(strncmp)(callee->name, "x86g_calculate_condition", 24) == 0) {
      ...
      //      ASSERT(cargs[1]->tag == Iex_RdTmp); op is not a Iex_RdTmp!
      ...
      d = unsafeIRDirty_0_N(0,
            ...
            mkIRExprVec_10(
               ...
               mkIRExpr_HWord(cargs[2]->Iex.RdTmp.tmp),
               mkIRExpr_HWord(cargs[3]->Iex.RdTmp.tmp),
               mkIRExpr_HWord(cargs[4]->Iex.RdTmp.tmp),
               cargs[0],
               cargs[1],
               cargs[2],
               cargs[3],
      ...
      //      ASSERT(cargs[0]->tag == Iex_RdTmp); op is not an Iex_RdTmp! 
      ...
      d = unsafeIRDirty_0_N(0,
            ...
            mkIRExprVec_9(
               mkIRExpr_HWord(lhsTmp),
               mkIRExpr_HWord(cargs[1]->Iex.RdTmp.tmp),
               mkIRExpr_HWord(cargs[2]->Iex.RdTmp.tmp),
               mkIRExpr_HWord(cargs[3]->Iex.RdTmp.tmp),
               cargs[0],
               cargs[1],
