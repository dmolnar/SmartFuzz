...
$curtime = time();
$newtestdir = "$prefix/tests/gensearch-$curtime-$inputfile"; 
...
print "Copying $inputfile to test directory $newtestdir \n"; 
system("cp $inputfile $newtestdir"); 
system("cp -r $prefix/tests/runtemplate/* $newtestdir"); 
chdir "$newtestdir"; 

print "Starting catchconv generational search in $newtestdir \n"; 
exec("perl $prefix/bin/gensearch.pl $inputfile $commandline"); 
