...
void isSetKeyOrigin(HWord key, HWord offset)
{
  ...
  isNode = VG_(malloc)("symbolicHash",sizeof(isIsSymbolicHashNode)); 
  isNode->key = key; 
  isNode->isOrigin = 1; 
...
void isIRTmpTmp(IRSB * bb, IRStmt * s, HWord BBCounter)
{
  ...
  lhskey = locToHashKey(s->Ist.WrTmp.tmp,BBCounter,TmpLoc); 
  rhskey = locToHashKey(rhs->Iex.RdTmp.tmp,BBCounter,TmpLoc); 
...
void isIRPut(IRSB * bb, IRStmt * s, HWord BBCounter)
{
  ...
  switch(data->tag)
  {
    ...
    case (Iex_RdTmp):
      ...
      numBytes = numBytesOfTmpvar(bb, data->Iex.RdTmp.tmp); 
      rhskey = locToHashKey(data->Iex.RdTmp.tmp,BBCounter,TmpLoc);
      d = unsafeIRDirty_0_N(0,
			    "isFlowToReg",
			    &isFlowToReg,
			    mkIRExprVec_3(
...
void isIRStore(IRSB * bb, IRStmt * s, HWord BBCounter)
{
  ...
  switch (dataExpr->tag)
    ...
    case (Iex_RdTmp):
      numBytes = numBytesOfTmpvar( bb, dataExpr->Iex.RdTmp.tmp);
      rhskey = locToHashKey(dataExpr->Iex.RdTmp.tmp,BBCounter,TmpLoc); 
      break; 
