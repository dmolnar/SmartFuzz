...
PRE(sys_read)
{
   ...
   *flags |= SfMayBlock;
   curOffset = VG_(lseek)(ARG1, 0, VKI_SEEK_CUR); 
   PRINT("sys_read ( %ld, %#lx, %llu )", ARG1, ARG2, (ULong)ARG3);

   if (VG_(resolve_filename)(ARG1,fname,16384))
   {
     PRINT(" filename %s", fname); 
     VG_(resolve_filename)(ARG1, fname, 16384); 
     VG_(snprintf)(details, 16384,
		   "read(buf) offset %d fname ",
		   curOffset);
     VG_(strcat)(details, fname); 
     PRINT(" | Passthrough string: %s ", details); 
   }
   else
   {
     VG_(snprintf)(details, 16384,
		   "read(buf) offset %d fname (none)",
		   curOffset); 
     PRINT(" no filename |  Passthrough string: %s ", details); 
   }
   ...
   {
     PRE_MEM_WRITE(details, ARG2, ARG3 );
   }
}
