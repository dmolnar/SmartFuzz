  ...
  if (!node) 
    {
      ...
      node = (OgVarOfHashNode *)VG_(malloc)(sizeof(OgVarOfHashNode));
      node->loc = loc; 
...
int setTypeOf(HWord varname, IntType type)
{
  ...
  if (!node)
    { 
      ...
      node = (OgTypeOfHashNode *)VG_(malloc)(sizeof(OgTypeOfHashNode));
      node->varname = varname; 
...
int getStructOf (HWord basic_block, HWord struct_info)
{
  ...
  if (!node)
    {
      /* allocate node for loc, since we are setting VarOf(loc) */
      node = (OgStructHashNode *)VG_(malloc)(sizeof(OgStructHashNode));
  ...
  else
    {
      ...
      vassert(node->basic_block == basic_block);
      OgStructHashNode *new_node = (OgStructHashNode *)VG_(malloc)(sizeof(OgStructHashNode));
...
void ogAddBinopAliasAndTypeConstraints(IRSB * bb, IRStmt * st)
{
  ...
  vassert(st->tag = Ist_Tmp); 
  the_op = (HWord)st->Ist.Tmp.data->Iex.Binop.op;
  arg1 = st->Ist.Tmp.data->Iex.Binop.arg1;
  arg2 = st->Ist.Tmp.data->Iex.Binop.arg2; 
  cur_ctr = (HWord)(counter);

  /* Emit new type variable for lhs */
  lhs = (HWord)st->Ist.Tmp.tmp;
  ...
  if ((arg1->tag == Iex_Tmp && arg2->tag == Iex_Const) 
      || (arg1->tag == Iex_Const && arg2->tag == Iex_Tmp))
    {
      if (arg1->tag == Iex_Tmp) tmpname = (HWord)arg1->Iex.Tmp.tmp;
      else if (arg2->tag == Iex_Tmp) tmpname = (HWord)arg2->Iex.Tmp.tmp;
...
void ogAddTmpConstraints(IRSB * bb, IRStmt * st, IRExpr * rhs) 
{
  ...
  switch (rhs->tag)
    ...
    case (Iex_Tmp) :
      ...
      ogAddTmp2TmpAliasHelper(bb,st->Ist.Tmp.tmp,rhs->Iex.Tmp.tmp); 
      break;
    case (Iex_Get) :
      ogAddTmp2GetAliasHelper(bb,st->Ist.Tmp.tmp, rhs); 
      break;
    case(Iex_Load):
      ogAddTmp2LoadAliasHelper(bb,st->Ist.Tmp.tmp,rhs->Iex.Load.addr);
      break;
    case (Iex_CCall):
      ogAddCCallConstraints(bb, st->Ist.Tmp.tmp,rhs);
      break; 
    case (Iex_Binop) :
      {
...
void ogAddPutConstraints(IRSB * bb, Int offset, IRExpr * data)
{
  ...
  switch (data->tag)
    ...
    case(Iex_Tmp):
      ...
      lhs_name = (HWord)data->Iex.Tmp.tmp; 
...
void ogAddStoreConstraints(IRSB * bb, IRExpr * addr, IRExpr * data)
{
  ...
  switch (addr->tag) 
	...
	{
	case (Iex_Tmp):
	  tmpname = (HWord) data->Iex.Tmp.tmp; 
