...
void cgEmitExitTmpConstraints(HWord tmpName, HWord tagVal, HWord condVal, HWord destAddr)
{
  ...
  newPCnode = (cgPathCondHashNode *)VG_(malloc)(sizeof(cgPathCondHashNode)); 
    ...
    case (Iex_RdTmp):
	  switch (rhs->tag)
	    ...
	    case (Iex_RdTmp):
	      ...
	      addrHword = (HWord)addr->Iex.Tmp.tmp;
	      rhsHword = (HWord)rhs->Iex.Tmp.tmp;
  ...
  switch (rhs->tag)
    ...
    case (Iex_Get):
      getOffset = (HWord) rhs->Iex.Get.offset; 
       ...
       lhsexpr->tag = Iex_RdTmp;
       lhsexpr->Iex.Tmp.tmp = (IRTemp)s->Ist.Tmp.tmp;
	...
	{
	case (Iex_RdTmp):
	  rhsTmpName = addrExpr->Iex.Tmp.tmp;
    ...
    default:
      if (ca_output_comment)
      {
	VG_(printf)("XXX Warning : Unhandled Ist_Tmp : ");
	ppIRStmt(s); 
	VG_(printf)("\n");
      } 
    }
    ...
    case (Iex_RdTmp):
      ...
      d = unsafeIRDirty_0_N(0,
			    ...
			    mkIRExprVec_4( 
					  ...
					  mkIRExpr_HWord(cgBBCounter),
					  IRExpr_Tmp(exitGuard->Iex.RdTmp.tmp),
