  ...
  while (! VG_(get_line)(val,line,sizeof(line)) )
    {
      ...
      regionBase = VG_(atoll_base)(line,10); 
	...
	{ i++; }
      ...
      regionSize = VG_(atoll_base)(&line[i+1],10); 
...
Bool ca_process_cmd_line_option(Char* arg)
{
  ...
  else if (VG_CLO_STREQ(arg, "--concretize-indices=yes"))
  {
     ca_mem_handling = CA_concretize_indices;
  }
  else if (VG_CLO_STREQ(arg, "--suppress-arrays=yes"))
  {
     ca_mem_handling = CA_suppress_arrays; 
  }
  else if (VG_CLO_STREQ(arg, "--full-mem=yes"))
  {
    ca_mem_handling = CA_full;
  }
  else
  {
