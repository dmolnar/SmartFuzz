...
void cgAddTmpUnopConstraints(IRBB * bb, IRStmt * s)
{
  IRExpr * rhs; 
  IRExpr * arg; 
  IROp theop; 
  ...
  vassert(arg->tag == Iex_Tmp || arg->tag == Iex_Const); 
  ...
  case (Iex_Tmp):
    ...
    extra_args = VG_(malloc)(sizeof(cgArgs_EmitUnopTmp));
